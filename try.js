...
function search() {
    ...
    if (result.length > 0) {
        for (course in result) {
            ...
            var row;
            if (camp === "Swarthmore") {
                row = `<tbody><tr class='tr' onclick='cSwap(this)' data-toggle="buttons-toggle"><td  height="50">${name}</td><td>${id}</td><td>${num}</td><td>${time}</td><td>${prof}</td><td>${camp}</td></tr></tbody>`
                $(table).append(row);
