...
function find(searchText, semester, campuses) {
  /*given array of string, chosen semester, and chosen campuses, return list of courses within conditions*/
  result = [];
  for (campus in campuses) {
...
function search() {
  ...
  var tableHeader = '<thead><tr><th style="text-align: center;">Course Name</th><th style="text-align: center;">Course Number</th><th style="text-align: center;">Time Offered</th><th style="text-align: center;">Instructor</th><th style="text-align: center;">Campus</th></tr></thead>'
  ...
  if (result.length > 0) {
    for (course in result) {
      ...
      var time = result[course]['Time And Days'];
      var prof = result[course]['Instructor'];
      var camp = result[course]['Campus'];
      var row = `<tbody><tr><td>${name}</td><td>${num}</td><td>${time}</td><td>${prof}</td><td>${camp}</td></tr></tbody>`
      $(table).append(row)
    }
  }
  else{
